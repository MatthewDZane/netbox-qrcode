...
def get_qr_text(size, text, font='ArialMT'):
    ...
    flag = True
    while flag:
        file_path_test = resource_listdir(__name__, 'fonts'.format(font))

        file_path = resource_stream(__name__, 'fonts/{}.ttf'.format(font))

        # try:
        fnt = ImageFont.truetype(r'/usr/share/fonts/ttf-ubuntu-font-family/Ubuntu-B.ttf', font_size)
        # except Exception:
    ...
    return img, file_path, file_path_test
